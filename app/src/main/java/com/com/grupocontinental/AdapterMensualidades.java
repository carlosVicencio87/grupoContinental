...
public class AdapterMensualidades extends ArrayAdapter<String> {
    private Activity activity;
    private ArrayList data;
    public Resources res;
    SpinnerModel spinnerModel=null;
    LayoutInflater inflater;
    public AdapterMensualidades(Principal activitySpinner, int textViewResourceId, ArrayList objects, Resources resLocal)
    {
    ...
    public View getCustomView(int position, View convertView, ViewGroup parent)
    {
        ...
        View row = inflater.inflate(R.layout.lista_mensualidades,parent,false);
        spinnerModel = null;
        spinnerModel= (SpinnerModel) data.get(position);
        TextView label = (TextView) row.findViewById(R.id.lista_pagos_mensualidades);
        //ImageView iconoTipo = (ImageView) row.findViewById(R.id.imagenSpinner);
        if (position==0)
        {
            label.setText("Selecciona una opcion");
        }
        else
        {
